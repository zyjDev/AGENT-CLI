...
// API基础配置
// 原值（课程作者服务器，2026-09-20 实测已不可达：8s 超时 / HTTP 000）：
//     baseUrl: 'http://115.190.107.206:8099/api/v1/agent'
// 改为派生当前页面主机名 —— 这样用 localhost 或 127.0.0.1 打开都能连上本地后端，
// 且两个来源都已在本项目 WebCorsConfig 的 allowed-origins 白名单内。
const API_CONFIG = {
    baseUrl: `http://${window.location.hostname}:8099/api/v1/agent`,
    headers: {
