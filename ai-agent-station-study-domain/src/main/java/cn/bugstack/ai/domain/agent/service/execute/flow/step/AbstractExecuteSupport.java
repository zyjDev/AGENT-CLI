...
public abstract class AbstractExecuteSupport extends AbstractMultiThreadStrategyRouter<ExecuteCommandEntity, DefaultFlowAgentExecuteStrategyFactory.DynamicContext, String> {
    ...
    /**
     * 记录一次降级：链路并不中断（Fail-Fast 的节点不会走到这里），
     * 但要把「哪些环节是兜底产出」记下来，最终总结里如实告知用户，
     * 避免把降级内容伪装成模型的结论。
     */
    protected void rememberDegrade(DefaultFlowAgentExecuteStrategyFactory.DynamicContext dynamicContext,
